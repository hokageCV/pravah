...
export async function signup(c: Context) {
  ...
  try {
    ...
    return c.json(
      {
        error:
          error instanceof Error ? error.message : 'Error on creating user',
      },
...
export async function profile(c: Context) {
  ...
  try {
    ...
    if (!decoded)
      return c.json({ error: 'Invalid token' }, HttpStatusCodes.UNAUTHORIZED);
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while fetching user data',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
