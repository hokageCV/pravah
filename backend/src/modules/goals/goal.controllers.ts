...
export async function create(c: Context) {
  ...
  try {
    ...
    if (existingGoals.length >= 3)
      return c.json(
        { error: 'A habit can have at most 3 goals' },
        HttpStatusCodes.CONFLICT,
      );

    let [newGoal] = await db
      .insert(goals)
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while creating goal',
      },
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while fetching goals',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
...
export async function show(c: Context) {
  ...
  try {
    let [goal] = await db.select().from(goals).where(eq(goals.id, id)).limit(1);
    if (!goal)
      return c.json({ error: 'Goal not found' }, HttpStatusCodes.NOT_FOUND);
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while retrieving goal',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while updating goal',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
...
export async function destroy(c: Context) {
  ...
  try {
    let result = await db.delete(goals).where(eq(goals.id, id)).run();
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while deleting goal',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
