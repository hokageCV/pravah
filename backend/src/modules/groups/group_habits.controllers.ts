...
export async function create(c: Context) {
  ...
  if (!parseResult.success) {
    console.error(
      '❌ Group Habit insert validation failed:',
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while creating association.',
      },
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while fetching association.',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
...
export async function destroy(c: Context) {
  ...
  if (!habitId || Number.isNaN(habitId))
    return c.json(
      { error: 'Invalid habit ID in request body' },
  ...
  try {
    ...
    if (habit.userId !== currentUserId)
      return c.json(
        { error: 'Unauthorized: only habit owner can remove habit' },
    ...
    let result = await db
      .delete(groupHabits)
      .where(
        and(eq(groupHabits.groupId, groupId), eq(groupHabits.habitId, habitId)),
      )
      .run();
