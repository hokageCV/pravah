...
export async function create(c: Context) {
  ...
  if (!parseResult.success) {
    console.error(
      '❌ Group Member insert validation failed:',
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while creating membership',
      },
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while fetching membership',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while fetching members with habits',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
...
export async function destroy(c: Context) {
  ...
  if (!userId || Number.isNaN(userId))
    return c.json(
      { error: 'Invalid user ID in request body' },
  ...
  try {
    ...
    if (!group)
      return c.json({ error: 'Group not found' }, HttpStatusCodes.NOT_FOUND);
    if (group.ownerId !== currentUserId)
      return c.json(
        { error: 'Unauthorized: only group owner can remove members' },
        HttpStatusCodes.FORBIDDEN,
      );
    if (userId === currentUserId)
      return c.json(
        { error: 'Owner cannot remove themselves from group membership' },
        HttpStatusCodes.BAD_REQUEST,
      );
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while deleting membership',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
