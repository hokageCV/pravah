...
export async function create(c: Context) {
  ...
  if (!parseResult.success) {
    console.error(
      '❌ HabitLog insert validation failed:',
  ...
  try {
    ...
    if (constraint?.type === 'unique')
      return c.json(
        { error: 'You have already logged this habit for today.' },
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while fetching habit logs',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
...
export async function show(c: Context) {
  ...
  if (!id || Number.isNaN(id))
    return c.json(
      { error: 'Invalid or missing habit log ID' },
      HttpStatusCodes.BAD_REQUEST,
    );

  try {
    let [habitLog] = await db
      .select()
      .from(habitLogs)
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while retrieving habit log',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
...
export async function update(c: Context) {
  ...
  if (!parseResult.success) {
    console.error(
      '❌ Habit log insert validation failed:',
      parseResult.error.format(),
    );
    return c.json(
      { error: 'Required fields missing' },
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while updating habit log',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
...
export async function destroy(c: Context) {
  ...
  try {
    let result = await db.delete(habitLogs).where(eq(habitLogs.id, id)).run();
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while deleting habit log',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Some error while fetching with habit logs',
      },
      HttpStatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
