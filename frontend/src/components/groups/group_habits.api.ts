...
export async function searchHabits(groupId: number, query: string): Promise<Habit[]> {
  let result = await safeFetch({ url: `${BASE_URL}/groups/${groupId}/habits/search?query=${query}`, method: 'GET' })
  return result.data;
}

export async function addHabitToGroup(groupId: number, habitId: number): Promise<GroupHabit> {
  let data = { habitId }
